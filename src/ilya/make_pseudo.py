...
def get_top_n_checkpoints(model_name, num_checkpoints=3):
    ...
    scores = np.array([name[:-4].split('_')[-1] for name in all_checkpoints], dtype=float)
    indices = np.argpartition(scores, num_checkpoints)[:num_checkpoints]
    checkpoints_names = [all_checkpoints[idx] for idx in indices]
    paths = [config.CHECKPOINTS_DIR.format(model_name) + '/' + name for name in checkpoints_names]
    return paths
...
def main():
    ...
    proba_df = predict_utils.predict_gmean_ensemble(checkpoints_paths, config.TEST_DIR)
