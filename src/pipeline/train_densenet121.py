...
from mymodels.densenet import densenet121
...
def load_train_ids():
    ...
    pseudo = [
        fn.replace('/home/ikibardin/Documents/Kaggle/camera-model-identification/data', '/media/devbox/ssd2/dataset')
        for fn in pseudo]
    ids_train = list(pd.read_csv('../input/cleaner.csv')['fns']) \
                + list(kaggle_train_meta['filename']) + 3 * list(pseudo)
...
def load_model(args):
    ...
    model = densenet121(num_classes=len(utils.CLASSES), pretrained=args.use_imagenet_weights)
