...
def load_train_ids():
    ...
    ids_train = list(pd.read_csv('../input/cleaner.csv')['fns']) \
                + list(kaggle_train_meta['filename']) + 2 * list(pseudo)
...
def load_model(args):
    ...
    model = densenet201(num_classes=len(utils.CLASSES), pretrained=args.use_imagenet_weights)
...
def main():
    ...
    if not args.test:
        ...
        ids_val = pd.read_csv('../input/last_day_val.csv')['filename']
        run_training(MODEL_NAME, model, ids_train, ids_val, args.crop_size, args.batch_size,
                     args.workers, args.learning_rate, args.max_epoch, args.patience, args.steps, d4=True)
    else:
