...
class NASNetALarge(nn.Module):
    ...
    def __init__(self, num_classes=1001, input_size=331):
        ...
        self.last_linear = nn.Linear(4032 + 1, self.num_classes)
    ...
    def logits(self, features, aug):
        x = self.relu(features)
        x = self.avg_pool(x)
        x = x.view(x.size(0), -1)
        x = self.dropout(x)
        x = self.last_linear(torch.cat((x, aug), 1))
        return x

    def forward(self, input, aug):
        x = rotate_channels(input)
        x = self.features(x)
        x = self.logits(x, aug)
        return x
...
def nasnetalarge(num_classes=1001, pretrained='imagenet', input_size=331):
    r"""NASNetALarge model architecture from the
    `"NASNet" <https://arxiv.org/abs/1707.07012>`_ paper.
    """
    if input_size != 331:
        print('Warning: default input size is 331, got {}'.format(input_size))
    if pretrained:
        ...
        model = NASNetALarge(num_classes=num_classes, input_size=input_size)
        model = load_pretrained_weights_no_fc(model, settings)
