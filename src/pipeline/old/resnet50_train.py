...
from mymodels.custom_models import ResNet
...
if not (args.test or args.test_train):
    ...
    if not args.extra_dataset:
        ...
        # train_meta = pd.read_csv('../input/external_clean.csv')
        # train_meta = train_meta[train_meta['filename'] != '../../data/extern_full/Motorola-Nexus-6/31137191902_7a70385230_o.jpg']
        # train_meta = train_meta[train_meta['filename'] != '../../data/extern_full/Motorola-Nexus-6/31166718881_14888bff02_o.jpg']
        # big_mask = np.logical_and((np.array(train_meta['width']) >= 1024), (np.array(train_meta['height']) >= 1024))
        ids_train = pd.read_csv('../input/cleaner.csv')['fns']
    ...
    valid_loader = DataLoader(val_dataset, batch_size=args.batch_size // 4, sampler=val_sampler,
                              num_workers=num_workers, pin_memory=True, collate_fn=default_collate_unsqueeze)

    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate)
    scheduler = ReduceLROnPlateau(optimizer, factor=0.5, patience=2, min_lr=1e-9, epsilon=1e-5, verbose=1, mode='min')
    ...
    with conditional(args.test, open(csv_name, 'w')) as csvfile:
        ...
        if args.test:
            ...
            df = pd.DataFrame(data=df_data, columns=CLASSES + ['fname'])
            df.to_hdf('{}_test_pr.h5'.format(experiment_name), 'prob')
