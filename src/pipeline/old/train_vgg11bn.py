...
from mymodels.vgg11bn import vgg11_bn_modified
...
# MAIN
model = vgg11_bn_modified(num_classes=len(CLASSES), pretrained=args.use_imagenet_weights)
...
if not (args.test or args.test_train):
    ...
    valid_loaders = {
        ...
        'manip': DataLoader(val_dataset_manip, batch_size=args.batch_size // 4,
                            num_workers=num_workers, pin_memory=True)  # , collate_fn=default_collate_unsqueeze)
    }
    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate)
    scheduler = ReduceLROnPlateau(optimizer, factor=0.5, patience=0, min_lr=1e-9, epsilon=1e-5, verbose=1, mode='min')
    ...
    with conditional(args.test, open(csv_name, 'w')) as csvfile:
        ...
        if args.test:
            ...
            df.to_hdf('{}_test_pr.hdf'.format(experiment_name), 'prob')
